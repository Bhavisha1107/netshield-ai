...
export default function DashboardPage() {
  ...
  const runPrediction = async () => {
    try {
      ...
      if (!latestTraffic?.features || latestTraffic.features.length !== 42) {
        setError(
          "This traffic record has no model features. Re-ingest the traffic data, then run the prediction again."
        );
        return;
      }
      ...
      const response = await api.post(
        "/prediction/predict",
        {
          features: latestTraffic.features,
          source_ip:
